function autoEmail() {
  ...
  while (matchingFilesSend.hasNext()) {
    ...
    for (const element of bodyEmailSearchRange) {
      ...
      if (email) {
        mailAgency = email.getElement().getText().slice(email.getStartOffset(), email.getEndOffsetInclusive() + 1);
        break;
      }
    }
    if (mailAgency) {
      ...
      console.log('No agency email found, PDF copy will be prepared for printing');
