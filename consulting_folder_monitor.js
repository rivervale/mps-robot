function checkConsultingFolder() {
  ...
  // Search for case sheet(s) with 'mpdone' in body; the MP should input 'mpdone' when done with a case
  ...
  } else { // Exit the whole function if no completed cases found
    console.log('No completed cases found. Exiting');
    return;
  }

  while (diagnosedCases.hasNext()) {
    let caseSheet = diagnosedCases.next();
